...
namespace VoxelToolkit
{
    ...
    public struct QuadGenerationJob : IJob
    {
        ...
        [WriteOnly] public NativeArray<Quad> OpaqueQuads;
        [WriteOnly] public NativeArray<Quad> TransparentQuads;
        public NativeArray<int> QuadsCount;
        ...
        private int GetLineLength(int4 position, int4 direction, int material, FaceOrientation orientation, int4 size, int chunkSizeSquared)
        {
            ...
            for (; math.all(endPosition < size); endPosition += direction)
            {
                ...
                if (currentFace.Material != material)
                    break;
            ...
            for (var x = 0; x < ChunkSize; x++)
            {
                for (var y = 0; y < ChunkSize; y++)
                {
                    for (var z = 0; z < ChunkSize; z++)
                    {
                        ...
                        var length = GetLineLength(new int4(x, y, z, 0), firstDirection, face.Material,
                                                   expectedOrientation, size, chunkSizeSquared);
                        ...
                        {
                            var currentLength = GetLineLength(finalPosition, firstDirection, face.Material, expectedOrientation, size, chunkSizeSquared);
                        ...
                        var countIndex = isTransparent ? 1 : 0;
                        var target = isTransparent ? TransparentQuads : OpaqueQuads;

                        target[QuadsCount[countIndex]++] = new Quad(currentPosition.xyz, endPosition.xyz, expectedOrientation, face.Material);
